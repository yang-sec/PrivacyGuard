...
int remote_attest_enclave(int socket_num, int* p_challenger_type, enclave_info_t* enclave_info)
{
    ...
    printf("\n-------------------- New attestation --------------------\nChallenger Type = %u (0: DataOwner, 1: DataConsumer, 2: iDataAgent)\n", auth_msg->challenger_type);
...
int remote_attest_challenger(int sock_num, int type, uint8_t* secret, int secret_size)
{
    ...
    printf("\nProvisioning the 16-Byte DataOwner's secret key to CEE. SUCCESS!.\n");
...
{
    char buffer1[1000];
    sprintf(buffer1, "node App/txCheckContract.js 0x%s", contract_addr);
    printf("%s\n", buffer1);
    system(buffer1);
}
